...
(function () {
    ...
    Cache.prototype._createEntry = function (key, value) {
        ...
        this._cleanup(this._accessQueue);
    ...
    Cache.prototype._promoteEntry = function (entry, queue) {
        if (entry.prev(queue)/*is not head entry already*/) {
            entry.remove(queue);
     ...
     * @private
     */
    Cache.prototype._cleanup = function (queue) {
        var lruEntry = queue.tail;
        while (lruEntry && this._canReap()) {
            if (lruEntry.prev(queue)) {
                this._rmEntry(lruEntry.prev(queue), this._REMOVAL_CAUSE_C);
                lruEntry = lruEntry.prev(queue);
            }
        }
