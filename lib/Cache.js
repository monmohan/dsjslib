...
(function () {
    ...
    var _onLoad=function (cache,err, result) {
        if (!err) {
            try {
                ...
                cache._rmEntry(this, 'loading failed');
            }
        } else {
            this.onLoad.forEach(function (callback) {
                callback.apply(null, [err, result]);
    ...
    function _asyncGet(key, callback, suppressLoad) {
        ...
        if (entry) {
            ...
            if (!this.isExpired(entry)) {
                ...
                if (!suppressLoad && !entry.loading) {
                    entry.loading = true;
                    entry.onLoad.push(callback);
                    asyncLoad(cache, _onLoad.bind(entry), key, callback);
                }
                else {
                    cache._rmEntry(entry, cache._REMOVAL_CAUSE_E);
                    callback.apply(null, [null, null]);
                }
            }
        } else {
    ...
    function syncLoad(cache, onLoad, key) {
        var err,
            result = null,
    ...
    Cache.prototype.getSync = function (key) {
        ...
        if (entry) {
            if (!this.isExpired(entry)) {
                ...
                this.stats.missCount++;
                if (!suppressLoad) {
                    ret = syncLoad(cache, function (err, result) {
                        if (err)throw err;
                        cache._notify(entry, cache._REMOVAL_CAUSE_E);
